...
def _increment_list_counter(ilvl2count, ilvl: str) -> int:
    ...
    # when first encountering a list below the top level, assume top higher levels
    # are at least 1. This matches Word behavior for GitHub issue #31
    higher_levels = [str(x) for x in range(int_ilvl)]
    for level in higher_levels:
        ilvl2count[level] = max(ilvl2count[level], 1)

    deeper_levels = [x for x in ilvl2count.keys() if int(x) > int_ilvl]
